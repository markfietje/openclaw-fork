...
When Gateway status reports degraded SecretRef owners, doctor prints a **Secret runtime degradation** warning with every owner, its `cold` or `stale` state, affected config paths, redacted reason, and the `openclaw secrets reload` retry command.
