...
- Link favicons are OFF by default. When the operator enables `gateway.controlUi.automaticallyFetchFavicons` AND lists hosts under `gateway.controlUi.remoteImageHosts`, the authenticated Control UI requests favicons only for allowlisted hosts through the Gateway; the browser never contacts link destinations directly. Unlisted hosts render a local letter tile — no element with a `src`, no request. The Gateway requests only each public hostname's HTTPS `/favicon.ico`, with strict DNS-pinned SSRF checks on the original URL and every redirect plus bounded time, bytes, concurrency, and image validation; the SSRF guard stays enforced even for allowlisted hosts. Private, internal, and IP-literal destinations are rejected. This discloses allowlisted hostnames and the Gateway's network address to those sites. Leave the setting off (default) or the host list empty to prevent all favicon route requests and destination fetches.
- Remote images in rendered markdown are OFF by default in every render mode. A model-controlled `![](https://host/x.png)` renders as a labeled not-loaded fallback with zero network requests unless the operator opts in and allowlists the host (`remoteImageHosts`, exact hostnames, subdomains never implied). Inline `data:` image URIs render only when the decoded payload is at most 64 KiB; larger payloads degrade to the fallback. Bare URLs in prose remain linkified-but-inert: they render as links and do not fetch until clicked.
