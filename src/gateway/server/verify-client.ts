...
import { isLocalDirectRequest, isTrustedProxyAddress } from "../net.js";
...
type GatewayVerifyClientParams = {
  ...
  /** Runtime config snapshot getter. The gateway resolves inputs at request time. */
 ...
 * - Paired browser-copilot (chrome-extension) clients pass the generic origin
 *   check; the post-handshake admission path skips its generic origin check for
 *   them too (`hasCopilotExtensionOrigin`) and validates the exact origin against
 *   the paired device, so both gates stay in sync.
...
export function createGatewayVerifyClient(params: GatewayVerifyClientParams): GatewayVerifyClient {
  ...
  return (info, callback) => {
    ...
    // Canonical Chrome extension (browser copilot) origins pass through for
    // post-handshake device validation, matching the admission path's
    // `hasCopilotExtensionOrigin` skip.
