...
describe("gateway verifyClient", () => {
  ...
  it("passes canonical Chrome extension (browser copilot) origins for post-handshake validation", async () => {
    // The post-handshake admission path skips its generic origin check for paired
    // copilot clients (`hasCopilotExtensionOrigin`) and validates the exact origin
    // against the device. The pre-handshake gate must stay in sync.
    expect(
      await verifyOk({
        remoteAddress: "192.0.2.10",
  ...
  it("treats loopback with any unrecognized x-forwarded-* header as non-local", async () => {
    // P1 locality regression: the hand-rolled forwarded-header list missed
    // unrecognized x-forwarded-* variants (e.g. x-forwarded-server), so a proxied
    // loopback peer was classified local and admitted via the local-loopback origin
    // fallback. Canonical isLocalDirectRequest catches every x-forwarded-* header.
    expect(
      await verifyOk({
        remoteAddress: "127.0.0.1",
        origin: "http://127.0.0.1:18789",
        headers: { host: "127.0.0.1:18789", "x-forwarded-server": "edge.example.com" },
      }),
    ).toBe(false);
  });
  ...
  it("still accepts an allowlisted origin for a proxied loopback client", async () => {
    expect(
      await verifyOk({
        remoteAddress: "127.0.0.1",
        origin: "https://app.example.com",
        headers: { host: "127.0.0.1:18789", "x-forwarded-for": "198.51.100.7" },
      }),
    ).toBe(true);
  });

  it("re-evaluates the origin gate from the runtime getter on every request", async () => {
    // P1 regression: the gate was wired to the startup config snapshot, so an
    // allowlist change at runtime never reached pre-handshake admission. The
    // getter must be read per request (same mechanism the post-handshake path
    // uses), so a live write flips the verdict without a restart.
    let currentCfg = {
      ...baseCfg,
      gateway: { ...baseCfg.gateway!, controlUi: { allowedOrigins: [] } },
    } as unknown as OpenClawConfig;
    const verifyClient = createGatewayVerifyClient({
      log: { info: () => {}, warn: () => {} },
      getConfigSnapshot: () => currentCfg,
    });
    ...
    expect(await run("https://app.example.com")).toBe(false);
    currentCfg = {
      ...baseCfg,
      gateway: {
        ...baseCfg.gateway!,
    ...
    expect(await run("https://app.example.com")).toBe(true);
    currentCfg = {
      ...baseCfg,
      gateway: { ...baseCfg.gateway!, controlUi: { allowedOrigins: [] } },
    } as unknown as OpenClawConfig;
